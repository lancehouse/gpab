...
# directly is fine now, see gpab_trial/main.py), falling back to
# ~/PAB-gtktrial/<name>/ (a copy made via copy_trial_session.sh) if not found.
set -euo pipefail
cd "$(dirname "$0")/.."
...
if [ $# -ne 1 ]; then
    ...
    echo "Available trial-copy sessions in ~/PAB-gtktrial:" >&2
    ls "$HOME/PAB-gtktrial" >&2 2>/dev/null || echo "  (none)" >&2
    exit 1
fi
...
if [ -z "$SESSION_FILE" ]; then
    echo "No *_session.json found under $HOME/PAB/$NAME or $HOME/PAB-gtktrial/$NAME" >&2
    exit 1
fi
