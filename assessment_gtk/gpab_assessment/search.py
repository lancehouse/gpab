"""Search index builder and fuzzy scorer for the Ctrl+F jump-search feature —
GTK4 port of pab_assessment.search.

Every static table below (`_SECTION_SHORT`, `_SECTION_LABELS`, `_SUBSECTIONS`,
`_SUBSECTION_LABEL`, `_SUBSECTION_EXTRA`, `_FIELD_LABELS`,
`_ASSESSMENT_SECTION_IDS`, `_OBJ_KB_FIELDS`) and the fuzzy scorer
(`fuzzy_score`/`filter_entries`/`_snippet`) are lifted verbatim from the TUI
file — pure data and pure logic, no Textual imports there in the first
place. One field id is corrected: the TUI's Consent preferred-name Input
carried the Textual id "preferred_name_input", but this port's collect()/
load() key (and hence its TouchEntry's field_id) is the bare
"preferred_name" — every other id in these tables already equals the
storage key by construction (checked via CONVERSION_PLAN's round-trip
verification), so this is believed to be the only such case; a missing
match degrades gracefully (see app.py's _execute_jump) rather than crashing,
same as the TUI's own try/except-wrapped widget lookups.

Objective section ids keep the TUI's "obj:NN_x" prefix in these tables
unchanged (rather than being rewritten to this app's own bare ids) so this
file stays a close, diffable port — app.py's _execute_jump strips the
prefix at navigation time, mirroring assessment_view.py's own
`if entry.section_id.startswith("obj:")` branch exactly.

build_index() is a genuine GTK-side rewrite: the TUI queries a live Textual
DOM (`section.query(CheckButton)`, `section.query_one(f"#{widget_id}")`);
this app has no such id-addressed query, so `_iter_descendants`/
`_find_by_field_id` below are a small generic Gtk.Widget-tree walker used
in its place (matching by each widget's own `field_id` attribute, which
every clinical widget in widgets.py already carries).
"""
...
_SUBSECTIONS: list[tuple[str, str, str]] = [
    ...
    ("03_medical", "diff_spa",             "Differential: Spondyloarthropathy (SCREEN'D'EM)"),
...
_FIELD_LABELS: dict[str, tuple[str, str | None, str]] = {
    ...
    "diff_spa_action":       ("03_medical", "med_differential",   "Spondyloarthropathy (SCREEN'D'EM) action"),
    ...
    "goal_type_treatment_seeking": ("02_subjective", "subj_goals", "Goal orientation: Treatment-seeking"),
    "goal_type_self_sufficiency":  ("02_subjective", "subj_goals", "Goal orientation: Self-sufficiency"),
    "goal_type_social_validation": ("02_subjective", "subj_goals", "Goal orientation: Social validation"),
    # 07 Barriers
...
_OBJ_KB_FIELDS: list[tuple[str, str, str, str, str]] = [
    ...
    ("st_er_resist_l",    "shoulder", "obj:08_special", "ER Resistance Left",        "impingement infraspinatus subacromial SIS cluster"),
    ("st_er_resist_r",    "shoulder", "obj:08_special", "ER Resistance Right",       "impingement infraspinatus subacromial SIS cluster"),
    ("st_irls_l",         "shoulder", "obj:08_special", "IR Lag Sign Left",          "subscapularis internal rotation lag full thickness tear"),
    ("st_irls_r",         "shoulder", "obj:08_special", "IR Lag Sign Right",         "subscapularis internal rotation lag full thickness tear"),
    ("st_drop_is_l",      "shoulder", "obj:08_special", "Drop Sign (IS) Left",       "infraspinatus drop sign rotator cuff lag battery"),
    ("st_drop_is_r",      "shoulder", "obj:08_special", "Drop Sign (IS) Right",      "infraspinatus drop sign rotator cuff lag battery"),
    ("st_hornblower_l",   "shoulder", "obj:08_special", "Hornblower's Left",         "teres minor rotator cuff lag battery massive tear"),
    ("st_hornblower_r",   "shoulder", "obj:08_special", "Hornblower's Right",        "teres minor rotator cuff lag battery massive tear"),
    ("st_ac_ext_l",       "shoulder", "obj:08_special", "AC Resisted Extension Left","acromioclavicular joint resisted extension confirm cluster"),
    ("st_ac_ext_r",       "shoulder", "obj:08_special", "AC Resisted Extension Right","acromioclavicular joint resisted extension confirm cluster"),
    ("st_pass_comp_l",    "shoulder", "obj:08_special", "Passive Compression Left",  "SLAP tear labral passive compression Kim"),
    ("st_pass_comp_r",    "shoulder", "obj:08_special", "Passive Compression Right", "SLAP tear labral passive compression Kim"),
    ("st_bicip_groove_l", "shoulder", "obj:08_special", "Bicipital Groove Left",     "biceps tendinopathy palpation groove tenderness"),
    ("st_bicip_groove_r", "shoulder", "obj:08_special", "Bicipital Groove Right",    "biceps tendinopathy palpation groove tenderness"),
    ("st_adson_l",        "shoulder", "obj:08_special", "Adson's Left",              "thoracic outlet syndrome TOS scalene subclavian"),
    ("st_adson_r",        "shoulder", "obj:08_special", "Adson's Right",             "thoracic outlet syndrome TOS scalene subclavian"),
    ("st_roos_l",         "shoulder", "obj:08_special", "Roos/EAST Left",            "thoracic outlet syndrome TOS elevated arm stress test"),
    ("st_roos_r",         "shoulder", "obj:08_special", "Roos/EAST Right",           "thoracic outlet syndrome TOS elevated arm stress test"),
    ("st_wright_l",       "shoulder", "obj:08_special", "Wright's Left",             "thoracic outlet syndrome TOS hyperabduction pectoralis minor"),
    ("st_wright_r",       "shoulder", "obj:08_special", "Wright's Right",            "thoracic outlet syndrome TOS hyperabduction pectoralis minor"),
    ("st_crlf_l",         "shoulder", "obj:08_special", "CRLF Left",                 "cervical rotation lateral flexion TOS rule out cervical spine"),
    ("st_crlf_r",         "shoulder", "obj:08_special", "CRLF Right",                "cervical rotation lateral flexion TOS rule out cervical spine"),
    # ── Hip ──────────────────────────────────────────────────────────────────
...
def filter_entries(query: str, index: list[SearchEntry], max_results: int = 10) -> list[SearchEntry]:
...
# ---------------------------------------------------------------------------
# Widget-tree walking — GTK has no id-addressed query like Textual's
# `query_one(f"#{id}")`, so navigation matches on each widget's own
# `field_id` attribute instead (every clinical widget in widgets.py sets one).
# ---------------------------------------------------------------------------
...
def find_by_field_id(container, field_id: str):
    """First descendant of `container` whose `.field_id` equals `field_id`,
    or None. Shared by build_index()'s content-harvesting step and by
    app.py's _execute_jump (same lookup, two different callers)."""
    for w in _iter_descendants(container):
        if getattr(w, "field_id", None) == field_id:
            return w
    return None
...
_SECTION_ID_TO_NAME = {
    "01_consent": "consent", "02_subjective": "subjective", "03_medical": "medical",
    "04_pain_classification": "pain_classification", "05_outcome_measures": "outcome_measures",
    "06_diagnosis": "diagnosis", "07_barriers": "barriers", "08_rx_plan": "rx_plan",
}


def _current_text(win, sec_id: str, widget_id: str) -> str | None:
    if sec_id == "scratchpad":
        return win.notes_overlay.text
    name = _SECTION_ID_TO_NAME.get(sec_id)
    section = win._sections_by_name.get(name) if name else None
    if section is None:
        return None
    widget = find_by_field_id(section, widget_id)
    return getattr(widget, "text", None) if widget is not None else None


def build_index(win) -> list[SearchEntry]:
    """Build a fresh search index from live app state. `win` is the
    TrialWindow — it already holds every section object directly
    (`win._sections_by_name`), so unlike the TUI's build_index(app) there's
    no need to look anything up via a query first."""
    entries: list[SearchEntry] = []

    # 1 — Sections
    for sec_id, label in _SECTION_LABELS.items():
        entries.append(SearchEntry(
            display=label, match_text=label, section_id=sec_id,
            anchor_id=None, widget_id=None, kind="section",
        ))
    ...
    for sec_id, anchor_id, label in _SUBSECTIONS:
        ...
        entries.append(SearchEntry(
            ...
            section_id=sec_id, anchor_id=anchor_id, widget_id=None, kind="subsection",
        ))
    ...
    for sec_id in _ASSESSMENT_SECTION_IDS:
        ...
        if section is None:
            continue
        short = _SECTION_SHORT.get(sec_id, sec_id)
        for btn in _iter_descendants(section):
            if not isinstance(btn, CheckButton):
                continue
            field_id = getattr(btn, "field_id", None)
            if not field_id:
                continue
            btn_name = btn.base_label
            entries.append(SearchEntry(
                display=f"{short} › {btn_name}",
                match_text=f"{btn_name} {short}",
                section_id=sec_id, anchor_id=None, widget_id=field_id, kind="field",
            ))
    ...
    for widget_id, (sec_id, anchor_id, human_name) in _FIELD_LABELS.items():
        ...
        display = f"{short} › {subsec} › {human_name}" if subsec else f"{short} › {human_name}"
        entries.append(SearchEntry(
            display=display, match_text=f"{human_name} {short}",
            section_id=sec_id, anchor_id=anchor_id, widget_id=widget_id, kind="field",
        ))

    # 5 — Content entries (non-empty current field values)
    for widget_id, (sec_id, anchor_id, human_name) in _FIELD_LABELS.items():
        short = _SECTION_SHORT.get(sec_id, sec_id)
        text = _current_text(win, sec_id, widget_id)
        ...
        entries.append(SearchEntry(
            ...
            match_text=text,
            section_id=sec_id, anchor_id=anchor_id, widget_id=widget_id, kind="content",
        ))

    # 5b — CAL-CP diagnosis workups (dynamic: one entry per pain-site
    # workup, pointing at whichever is currently actionable — the intro
    # form before it's begun, or the current question tab after). widget_id
    # is a synthetic "__workup_intro__<wid>" / "__workup_current__<wid>"
    # marker (there's no single real widget to focus — the notebook page
    # itself IS the destination), parsed by app.py's _execute_jump rather
    # than resolved via find_by_field_id. A finished workup is skipped, same
    # as the TUI (nothing actionable left to jump to).
    dx_section = win._sections_by_name.get("diagnosis")
    if dx_section is not None:
        ...
        for wid, w in getattr(dx_section, "_workups", {}).items():
            if w.finished:
                continue
            if not w.intro_done:
                widget_id, human = f"__workup_intro__{wid}", "Start intro form"
            else:
                widget_id, human = f"__workup_current__{wid}", "Current question"
            entries.append(SearchEntry(
                display=f"{short} › {w.label} › {human}",
                match_text=f"{w.label} {human} diagnosis CAL-CP ICD-11 chronic pain",
                section_id="06_diagnosis", anchor_id=None, widget_id=widget_id, kind="field",
            ))
    ...
    for widget_id, region, sec_id, human_name, extra in _OBJ_KB_FIELDS:
        ...
        entries.append(SearchEntry(
            ...
            section_id=sec_id, anchor_id=None, widget_id=widget_id, kind="field",
        ))
